...
- Padrão operacional com Mission Control como “fonte visual”: usar **status + task_activities** para rastrear execução em tempo real evita que a tarefa “fique só no chat”.

## O que não funcionou / problemas
- Tentar consertar um deploy remendado (muitos processos órfãos, nginx configs quebrados, service workers) atrasou a entrega.
- `openclaw cron run` pode entrar em **gateway timeout** mesmo com o gateway “RPC probe ok”; precisa diagnóstico (e possivelmente restart/ajuste de scheduler). Enquanto isso, crons com delivery `announce` podem falhar com `cron announce delivery failed` — preferir `delivery.mode=none` (ou `bestEffort`) para rotinas internas.
- Mission Control task_activities aceita apenas `activity_type` do enum (`spawned|updated|completed|file_created|status_changed`). Qualquer outro (ex.: `note`) falha 400.
rega — restart do zero foi a solução.
